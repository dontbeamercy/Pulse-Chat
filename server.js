...
function formatCall(call) {
  const caller = getUser(call.callerId);
  const callee = getUser(call.calleeId);
  return {
    id: call.id,
    type: call.type,
    status: call.status,
...
function getUserActiveCall(userId) {
  for (const call of activeCalls.values()) {
    if ((call.callerId === userId || call.calleeId === userId) && call.status !== 'ended') return call;
  }
  return null;
}

function callPeer(call, userId) {
...
function endCall(callId, reason) {
  ...
  notifyUser(call.callerId, 'call_ended', payload);
  notifyUser(call.calleeId, 'call_ended', payload);
  activeCalls.delete(callId);
}

function endCallsForUser(userId, reason) {
  for (const [id, call] of [...activeCalls.entries()]) {
    if (call.callerId === userId || call.calleeId === userId) endCall(id, reason);
  }
}
...
async function handleApi(req, res) {
  ...
  if (pathname === '/api/calls' && req.method === 'POST') {
    try {
      ...
      setTimeout(() => {
        ...
        if (c && c.status === 'ringing') endCall(callId, 'timeout');
      }, 45000);
  ...
  if (callActionMatch) {
    ...
    if (action === 'accept' && req.method === 'POST') {
      if (call.calleeId !== user.id) return json(res, 403, { error: 'Только получатель может принять' });
      if (call.status !== 'ringing') return json(res, 409, { error: 'Звонок уже завершён' });
      call.status = 'active';
      notifyUser(call.callerId, 'call_accepted', { callId, call: formatCall(call) });
      notifyUser(call.calleeId, 'call_accepted', { callId, call: formatCall(call) });
      return json(res, 200, { call: formatCall(call) });
    }

    if (action === 'reject' && req.method === 'POST') {
      endCall(callId, call.calleeId === user.id ? 'rejected' : 'cancelled');
      return json(res, 200, { ok: true });
    }
    ...
    if (action === 'signal' && req.method === 'POST') {
      try {
        ...
        const peerId = callPeer(call, user.id);
        notifyUser(peerId, 'call_signal', {
          callId,
          fromUserId: user.id,
          signal: body.signal,
        });
